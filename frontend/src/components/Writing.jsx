...
const S = {
  ...
  btnGrade: { padding: '12px 32px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #f59e0b, #d97706)', color: 'white', fontWeight: 700, cursor: 'pointer', fontSize: '1rem', boxShadow: '0 4px 12px #f59e0b40', marginTop: 16 },
  ...
  suggestionsBox: { marginTop: 8, padding: '12px 16px', borderRadius: 10, background: '#eff6ff', border: '1px solid #bfdbfe' },
};

export default function Writing({ question, answer, onAnswer, result, grading, onGrade }) {
  ...
  const weaknesses = result?.areasForImprovement || [];
  ...
  return (
    <div style={S.container}>
      <div style={S.header}>
        ...
        <h2 style={S.title}>Luyện viết</h2>
      </div>
      ...
      <div style={{ fontSize: '1rem', fontWeight: 600, color: '#1a202c', marginBottom: 12 }}>
        {question.question}
      </div>
      ...
      <textarea
        ...
        placeholder="Viết bài của bạn ở đây. Hãy cố gắng viết ít nhất 50 từ..."
        ...
        <div style={S.gradingBox}>
          ...
          {suggestions.length > 0 && (
            <div style={S.suggestionsBox}>
              <div style={S.listTitle}>💡 Gợi ý cải thiện</div>
              {suggestions.map((s, i) => (
                <div key={i} style={S.listItem}>• {s}</div>
              ))}
            </div>
